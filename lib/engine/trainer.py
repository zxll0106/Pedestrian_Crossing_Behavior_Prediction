...
from lib.modeling.layers.cls_loss import binary_cross_entropy_loss, cross_entropy_loss, trn_loss,cvae_cross_entropy
...
def do_val(cfg, epoch, model, dataloader, device, logger=None, iteration_based=False):
    ...
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            total_action = torch.cat((target_action, target_future_action), dim=1)  # 128,20

            target_future_action = torch.stack([total_action[:, i + 1:i + 6] for i in range(target_action.shape[1])],
                                               dim=1)
            ...
            if cfg.MODEL.WITH_POSE:
                x_pose= batch['obs_pose'].to(device)
            else:
                x_pose = None
            
            act_det_scores, act_pred_scores, int_det_scores,_ ,KLD= model(x,
                                                                                bboxes,
                                                                                x_pose=x_pose,
                                                                                x_ego=ego_motion, 
                                                                                x_traffic=x_traffic,
            ...
            if cfg.MODEL.WITH_VAE==True or cfg.MODEL.WITH_CVAE==True:

                KLD_meter.add(KLD.item())
            ...
            if act_det_scores is not None:
                ...
                loss_act_det = cross_entropy_loss(act_det_scores, target_action)
                loss_act_det_meter.add(loss_act_det.item())
            ...
            if act_pred_scores is not None:
                ...
                if cfg.MODEL.WITH_CVAE==True:
                    loss_act_pred = cvae_cross_entropy(act_pred_scores, target_future_action)
                else:
                    loss_act_pred=cross_entropy_loss(act_pred_scores,target_future_action)
                loss_act_pred_meter.add(loss_act_pred.item())
...
def do_train_iteration(cfg, model, optimizer, 
    ...
    with torch.set_grad_enabled(True):
        ...
        # inference(cfg, 0, model, test_dataloader, device, logger=logger, iteration_based=True)
        for iters, batch in enumerate(tqdm(train_dataloader), start=1):
            ...
            target_future_action=torch.stack([total_action[:,i+1:i+6] for i in range(target_action.shape[1])],dim=1)
            ...
            if cfg.MODEL.WITH_VAE==True or cfg.MODEL.WITH_CVAE==True:
                loss=loss+cfg.SOLVER.KLD_WEIGHT*KLD
                KLD_meter.add(KLD.item())
                loss_dict['KLD']=KLD.mean()

            if act_det_scores is not None:
                loss_act_det=cross_entropy_loss(act_det_scores,target_action)
                loss += cfg.SOLVER.ACT_DET_WEIGHT*loss_act_det
                loss_act_det_meter.add(loss_act_det.item())
                loss_dict['loss_act_det_train'] = loss_act_det_meter.mean
            ...
            if act_pred_scores is not None:
                ...
                if cfg.MODEL.WITH_CVAE == True:
                    ...
                    loss_act_pred=cross_entropy_loss(act_pred_scores,target_future_action)
                loss += cfg.SOLVER.ACT_PRED_WEIGHT*loss_act_pred
                loss_act_pred_meter.add(loss_act_pred.item())
                loss_dict['loss_act_pred_train'] = loss_act_pred_meter.mean
            # intent loss
            if int_det_scores is not None:
                ...
                loss += cfg.SOLVER.INTENT_DET_WEIGHT*loss_intent
                loss_intent_meter.add(loss_intent.item())
                loss_dict['loss_int_det_train'] = loss_intent_meter.mean
            # weight
            if hasattr(model, 'param_scheduler'):
