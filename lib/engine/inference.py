...
def inference(cfg, epoch, model, dataloader, device, logger=None, iteration_based=False):
    ...
    # Collect outputs
    gt_actions, gt_intents, gt_future_actions = defaultdict(list), defaultdict(list), defaultdict(list)
    ...
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            # total_action = torch.cat((batch['obs_action'] ,batch['pred_action']), dim=1)  # 128,20
            #
            # target_future_action = torch.stack([total_action[:, i + 1:i + 6] for i in range(target_action.shape[1])],
            #                                    dim=1)
            # target_future_action=target_future_action.numpy()
            ...
            start = time.time()
            # flops,params=profile(model,inputs=(x,bboxes,x_pose,ego_motion,x_traffic,local_bboxes,masks))
            # flops,params=clever_format([flops,params],'%.3f')
            # print(flops,params)

            act_det_scores, act_pred_scores, int_det_scores, attentions,_= model(x,
                                                                                        bboxes,
                                                                                        x_pose=x_pose,
                                                                                        x_ego=ego_motion,
                                                                                        x_traffic=x_traffic, 
            ...
            # if len(total_times)==714:
            #     with open('total_time_all_iter2300_2.pkl', 'wb') as f:
    ...
    predictions_name=cfg.TEST.PREDICTION_NAME+'.pkl'
    with open(predictions_name,'wb') as f:
