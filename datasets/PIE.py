...
class PIEDataset(data.Dataset):
    def __init__(self, cfg, split):
        ...
        if self.cfg.MODEL.WITH_POSE:
            pedestrian_pose_set01_path='/extend/zxl/Intent_Estimation/PedestrianActionBenchmark-main/PedestrianActionBenchmark-main/data/features/pie/poses/pose_set01.pkl'
            pedestrian_pose_set02_path = '/extend/zxl/Intent_Estimation/PedestrianActionBenchmark-main/PedestrianActionBenchmark-main/data/features/pie/poses/pose_set02.pkl'
            pedestrian_pose_set03_path = '/extend/zxl/Intent_Estimation/PedestrianActionBenchmark-main/PedestrianActionBenchmark-main/data/features/pie/poses/pose_set03.pkl'
            pedestrian_pose_set04_path = '/extend/zxl/Intent_Estimation/PedestrianActionBenchmark-main/PedestrianActionBenchmark-main/data/features/pie/poses/pose_set04.pkl'
            pedestrian_pose_set05_path = '/extend/zxl/Intent_Estimation/PedestrianActionBenchmark-main/PedestrianActionBenchmark-main/data/features/pie/poses/pose_set05.pkl'
            pedestrian_pose_set06_path = '/extend/zxl/Intent_Estimation/PedestrianActionBenchmark-main/PedestrianActionBenchmark-main/data/features/pie/poses/pose_set06.pkl'

            self.pedestrian_pose ={}
            self.pedestrian_pose['set01']=pkl.load(open(pedestrian_pose_set01_path,'rb'))
            self.pedestrian_pose['set02'] = pkl.load(open(pedestrian_pose_set02_path, 'rb'))
            self.pedestrian_pose['set03'] = pkl.load(open(pedestrian_pose_set03_path, 'rb'))
            self.pedestrian_pose['set04'] = pkl.load(open(pedestrian_pose_set04_path, 'rb'))
            self.pedestrian_pose['set05'] = pkl.load(open(pedestrian_pose_set05_path, 'rb'))
            self.pedestrian_pose['set06'] = pkl.load(open(pedestrian_pose_set06_path, 'rb'))
        ...
        # if split == 'train':
        #     with open('PIE_train_set.pkl', 'wb') as f:
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.MODEL.WITH_TRAFFIC:
            if self.traffic_features is None:
                ret.update(self.extract_traffic_features_new(index, obs_bboxes))
            else:
        ...
        if self.cfg.MODEL.WITH_POSE:
            ret['obs_pose'] = []
            for pid, img_file in zip(pids, self.data['obs_image'][index]):
                set_name=img_file.split('/')[-3]
                video_name = img_file.split('/')[-2]
                image_name = img_file.split('/')[-1].split('.')[0] + '_' + pid[0]
                if image_name in self.pedestrian_pose[set_name][video_name]:
                    ret['obs_pose'].append(
                        torch.FloatTensor(self.pedestrian_pose[set_name][video_name][image_name]).unsqueeze(0))
                else:
                    ret['obs_pose'].append(
                        torch.FloatTensor(torch.zeros((1,36))))
    ...
    def extract_traffic_features_new(self, index, ped_bboxes):
        # NOTE: Nov 16, extract traffic features.
        keys = ['neighbor_bboxes',
                'neighbor_classes',
                'neighbor_ids',
        ...
        # ret = {}
        ret = {k: [torch.FloatTensor([])] for k in keys}
        ped_x_c, ped_y_c, ped_y_b = (ped_bboxes[:, 0] + ped_bboxes[:, 2]) / 2, (
                    ped_bboxes[:, 1] + ped_bboxes[:, 3]) / 2, ped_bboxes[:, 3]
        ...
        if len(self.data['obs_traffic_bbox'][index]) > 0:
            ...
            # ret['neighbor_orig'] = copy.deepcopy(neighbor_bboxes.numpy())
            neighbor_classes = torch.FloatTensor(self.data['obs_traffic_class'][index])
            neighbor_classes[neighbor_bboxes.max(dim=-1)[
                                 0] == 0] = -1  # NOTE: change the class to -1 if object does not exit on that frame.

            ret['neighbor_bboxes'] = [torch.cat([neighbor_bboxes,neighbor_classes.unsqueeze(-1)],dim=-1)]
            ret['neighbor_classes'] = [neighbor_classes]
            ret['neighbor_ids'] = [self.data['obs_traffic_obj_id'][index]]
        ...
        if len(self.data['obs_traffic_light_bbox'][index]) > 0:
            ...
            light_class = torch.FloatTensor(self.data['obs_traffic_light_class'][index])
            light_class[a.max(dim=-1)[0] == 0] = -1
            # a[..., 0], a[..., 1], a[..., 2], a[..., 3] = (a[..., 0] + a[..., 2]) / 2, (a[..., 1] + a[..., 3]) / 2, a[
            #     ..., 2] - a[..., 0], a[..., 3] - a[..., 1]
            # a[..., 0], a[..., 1] = a[..., 0] - ped_x_c.unsqueeze(0), a[..., 1] - ped_y_c.unsqueeze(0)
            ret['traffic_light'] = [torch.cat([a,
                                               torch.FloatTensor(self.data['obs_traffic_light_class'][index]).unsqueeze(
                                                   -1),
                                               torch.FloatTensor(self.data['obs_traffic_light_state'][index]).unsqueeze(
                                                   -1)
                                               ], dim=-1)]
            ret['traffic_light_classes'] = [light_class]
        if len(self.data['obs_traffic_sign_bbox'][index]) > 0:
            ...
            sign_class = torch.FloatTensor(self.data['obs_traffic_sign_class'][index])
            sign_class[a.max(dim=-1)[0] == 0] = -1
            # a[..., 0], a[..., 1], a[..., 2], a[..., 3] = (a[..., 0] + a[..., 0]) / 2, (a[..., 1] + a[..., 3]) / 2, a[
            #     ..., 2] - a[..., 0], a[..., 3] - a[..., 1]
            # a[..., 0], a[..., 1] = a[..., 0] - ped_x_c.unsqueeze(0), a[..., 1] - ped_y_c.unsqueeze(0)
            ret['traffic_sign'] = [torch.cat([a,
                                              torch.FloatTensor(self.data['obs_traffic_sign_class'][index]).unsqueeze(
                                                  -1),
                                              ], dim=-1)]
            ret['traffic_sign_classes'] = [sign_class]
        if len(self.data['obs_crosswalk_bbox'][index]) > 0:
        ...
        if len(self.data['obs_station_bbox'][index]) > 0:
            # station feature is [x_c_l_diff, x_c_r_diff, y_b_diff, x1, y1, x2, y2]
            a = torch.FloatTensor(self.data['obs_station_bbox'][index])
            # ret['station_orig'] = copy.deepcopy(a.numpy())64
            station_class = torch.ones((a.shape[0], a.shape[1]))
            station_class[a.max(dim=-1)[0] == 0] = -1
            ret['station'] = [a]
            ret['station_classes'] = [station_class]
    ...
    def load_extracted_features(self, pids, img_files):
        ...
        for t, img_path in enumerate(img_files):
            ...
            # tmp_path = '/extend/zxl/Intent_Estimation/PIEPredict-master/data/data/pie'
            #
            # data_path = os.path.join(tmp_path, self.split,
            #                          'features_context_pad_resize/vgg16_none',
            #                          sid, vid, img_id + '_' + pid + '.pkl')
